...
export default function ModuleCard({ module, progress, interactiveProgress, onClick }: ModuleCardProps) {
  ...
  return (
    <button onClick={onClick} className="w-full text-left group h-full">
      <article className="relative overflow-hidden rounded-3xl border border-white/60 bg-white/90 p-6 shadow-[0_14px_38px_rgba(15,23,42,0.08)] backdrop-blur transition duration-200 group-hover:-translate-y-0.5 group-hover:shadow-[0_20px_50px_rgba(15,23,42,0.12)] h-full flex flex-col">
        ...
        <div className="relative z-10 flex gap-5 h-full">
          <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl bg-gray-900/5 text-4xl">
            {module.icon}
          </div>
          <div className="flex-1 flex flex-col min-h-[160px]">
            ...
            <p
              className="text-sm text-gray-600 leading-relaxed min-h-[1.5rem]"
            >
              {module.description}
            </p>
            <div className="flex-1" />
            <div className="flex items-center gap-4 text-xs text-gray-500 mt-5 pt-3 border-t border-gray-100/50">
